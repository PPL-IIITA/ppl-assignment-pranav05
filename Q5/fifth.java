...
public class fifth {
    ...
    public static void main(String[] args) {
        ...
        j=0;
        for (k = 0; k < cb; k++) {
            if (j % 2 == 0 && g[j].stat.equals("single") && g[j].main_bud <= b[k].budget && b[k].stat.equals("single") && g[j].att >= b[k].min_att) {
                g[j].bf = b[k].name;
                b[k].gf = g[j].name;
                g[j].stat = "committed";
                b[k].stat = "committed";
            ...
            if (j % 2 != 0) {
                for (m = 0; m < cg; m++) {
                    if (g[m].stat.equals("single") && g[m].main_bud <= b[k].budget && b[k].stat.equals("single") && g[m].att >= b[k].min_att) {
                        g[m].bf = b[k].name;
                        b[k].gf = g[m].name;
                        g[m].stat = "committed";
                        b[k].stat = "committed";
        ...
        try {
            FileWriter fileOut = new FileWriter("happy.csv");
            /**
             * Emptying the file
             */
            fileOut.write("");
